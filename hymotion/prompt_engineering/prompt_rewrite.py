...
class PromptRewriter:
    def __init__(
        ...
        if host:
            ...
            if not model_path:
                ...
                model_path = os.path.join(base_dir, "GGUF", "Qwen3-8B-UD-Q5_K_XL.gguf")
    ...
    def _load_model(self):
        if self.llm is None:
            print(f">>> Loading prompter model from {self.model_path} via llama_cpp, offload_to_cpu={self.offload_to_cpu}")
            try:
                ...
                self.llm = Llama(
                    ...
                    n_ctx=8192,
                    n_batch=512,
                    n_gpu_layers=0 if self.offload_to_cpu else -1,
                    verbose=False,
                )
            except ImportError as e:
                raise ImportError(f"Please install llama-cpp-python. Error: {e}")
    ...
    def rewrite_prompt_and_infer_time(
        ...
        if self.host:
            ...
            print(f">>> [Prompter] Running LLM inference via llama_cpp...")
            ...
            print(f"\n>>> [Prompter] Raw LLM response finished.")

            try:
                # Remove reasoning blocks from thinking models to avoid parsing bad JSON
